...
class DataSet():
	...
	'''
	def __init__(self, folder, preprocess, wavelet, histEq, tophat, norm, resize, smooth, context, vgg):	
	...
	def get_side_patches(self, folder, preprocess, wavelet, histEq, tophat, norm, resize, smooth):
		train = np.load(folder + 'side_train_indx.npy').item()
		val = np.load(folder + 'side_val_indx.npy').item()
		test = np.load(folder + 'side_test_indx.npy').item()
		if resize is True:
			train['imgs'] = train['imgs'][:,32:96,32:96]
			val['imgs'] = val['imgs'][:,32:96,32:96]
			test['imgs'] = test['imgs'][:,32:96,32:96]
	...
	def createOnehot(self,data, percentage):
		#WRONG! Need different percentages!
	...
	def preprocess(self,imgs, wavelet, histEq, tophat = False, norm = '0to1', smooth = False ):
		...
		n = imgs.shape[0]
		if self.vgg_flag == True:
			imgs_new = np.zeros(shape = (n, 224,224, 3))
		else:
			imgs_new = np.zeros(shape = (n, 128, 128))
		if wavelet is True:
			...
			for i in range(n):
				...
				if smooth is True:
					...
					# pdb.set_trace()
					img = cv2.medianBlur((img/256).astype('uint8'),7);
				if norm == '0to1':
					img = img - np.min(img)
