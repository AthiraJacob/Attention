...
data = DataSet(dataFold, preprocess = True, wavelet = False, histEq = False, tophat = False, 
    norm = '0to1', resize = FLAGS.resize, smooth = False, context = False, vgg = False)
...
if os.path.isfile(model_name) and FLAGS.restore is True:
    ...
    model_1.add(Conv2D(f, (3, 3), activation='relu'))
    model_1.add(Conv2D(f, (3, 3), activation='relu'))
    model_1.add(Conv2D(f, (5, 5), activation='relu'))
    model_1.add(MaxPooling2D((2, 2), strides=(2, 2)))

    model_1.add(Conv2D(f*2, (5, 5), activation='relu'))
    model_1.add(Conv2D(f*2, (5, 5), activation='relu'))
    model_1.add(Conv2D(f*2, (5, 5), activation='relu'))
    model_1.add(MaxPooling2D((2, 2), strides=(2, 2)))

    model_1.add(Conv2D(f*4, (5, 5), activation='relu'))
    model_1.add(Conv2D(f*4, (5, 5), activation='relu'))
    # model_1.add(Conv2D(f*4, (5, 5), activation='relu'))
    # model_1.add(MaxPooling2D((2, 2), strides=(2, 2)))
