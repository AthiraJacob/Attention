...
data = DataSet(dataFold, preprocess = True, wavelet = False, histEq = False, tophat = False, 
    norm = 'm1to1', resize = FLAGS.resize, smooth = False, context = False, vgg = False)
...
if FLAGS.use_all_train is True:
    side_train, _, _ = data.get_side_patches(dataFold, preprocess = True, wavelet = False, 
        histEq = False, tophat = False, norm = 'm1to1', resize = FLAGS.resize, smooth = False)
    # pdb.set_trace()
...
if os.path.isfile(model_name) and FLAGS.restore is True:
    ...
    model_1.add(GaussianNoise(0.005, input_shape = input_shape))
    model_1.add(Conv2D(f, (3, 3), activation='relu', padding='same'))
    model_1.add(Conv2D(f, (3, 3), activation='relu', padding='same'))
    model_1.add(Conv2D(f, (5, 5), activation='relu', padding='same'))
    model_1.add(MaxPooling2D((2, 2), strides=(2, 2)))

    model_1.add(Conv2D(f*2, (5, 5), activation='relu', padding='same'))
    model_1.add(Conv2D(f*2, (5, 5), activation='relu', padding='same'))
    model_1.add(Conv2D(f*2, (5, 5), activation='relu', padding='same'))
    model_1.add(MaxPooling2D((2, 2), strides=(2, 2)))

    model_1.add(Conv2D(f*4, (5, 5), activation='relu', padding='same'))
    model_1.add(Conv2D(f*4, (5, 5), activation='relu', padding='same'))
    model_1.add(Conv2D(f*4, (5, 5), activation='relu', padding='same'))
    model_1.add(MaxPooling2D((2, 2), strides=(2, 2))) #16

    
    print("Creating attention mechanism")
    input_shape = (16,16,f*4)
    ...
    features = Dropout(0.3)(h_proj) #N x Lx D
    h_att = Dense(1, activation='linear', use_bias = False, kernel_initializer = 'glorot_uniform')(features) #N x Lx 1
    h_att = Reshape((L,))(h_att)
    alpha = sparsemax()(h_att)
    ...
    h1 = Dropout(0.5)(h1)
    h1 = Dense(128, activation='relu', kernel_initializer = 'glorot_uniform')(context)
    h1 = Dropout(0.5)(h1)
    y = Dense(1, activation='linear', kernel_initializer = 'glorot_uniform')(h1)
...
if FLAGS.inspect is False:
    ...
    act_train = train[field]/100.0
    # pdb.set_trace()
    # pred = {'pred_test' : pred_test, 'act_test': act_test, 'pred_val': pred_val, 'act_val': act_val}
    pred = [ pred_val, act_val, pred_test, act_test, pred_train, act_train]
